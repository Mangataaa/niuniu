...
class NiuniuPlugin(Star):
    def __init__(self, context: Context, config: dict = None):
        ...
        # 统一所有message_gen引用
        # 初始化冷却时间记录器
        self.cool_down_time = {}  # 计数{group_id: {user_id:{last_dajiao_time:  rename_time:}}
        # self.last_dajiao_time = defaultdict(dict)    # 群组打胶时间 {group_id: {user_id:{last_dajiao_time: invite_count: last_compare_time}}
        self.invite_count = defaultdict(dict)  # 邀请计数 {group_id: {user_id: (timestamp, count)}}
        self.last_compare_time = defaultdict(dict)  # 比划时间 {group_id: {user_id: {target_id: timestamp}}}
    ...
    async def register_niuniu(self, event: AstrMessageEvent):
        ...
        yield event.plain_result(template.format(
            ...
            cowname=new_user_data["nickname"],
            length=length
        ))
    ...
    async def dajiao(self, event: AstrMessageEvent):
        ...
        if group_id and user_id in group_data:
            ...
            group_ = self.cool_down_time.setdefault(group_id, {})
            user_ = group_.setdefault(user_id, {})
            last_dajiao_time = user_.get("last_dajiao_time", 0)
            time_diff = current_time - last_dajiao_time

            # 十分钟内不允许打胶
            dajiao_cd = 10
            MIN_COOLDOWN = dajiao_cd * 60
            if time_diff < MIN_COOLDOWN:
                template = self.message_gen.get_template('dajiao', 'cooldown_messages')
                yield event.plain_result(template.format(nickname=sender_nickname, dajiao_cd=dajiao_cd))
                return

            # 超过十分钟但低于三十分钟，越接近十分钟越容易失败
            THIRTY_MINUTES = 3 * dajiao_cd * 60
            if time_diff < THIRTY_MINUTES:
                ...
                if self.check_probability(failure_probability):
                    ...
                    template = self.message_gen.get_template('dajiao', 'diff_decrease_messages')
                    message = template.format(nickname=sender_nickname, change=positive_change)
                else:
                    change = random.randint(0, max_change)
                    if change > 0:
                        template = self.message_gen.get_template('dajiao', 'diff_increase_messages')
                        message = template.format(nickname=sender_nickname, change=change)
                    else:
                        template = self.message_gen.get_template('dajiao', 'diff_no_effect_messages')
                        message = template.format(nickname=sender_nickname)

                # 更新长度和统计
                user_info["length"] += change
                user_info['dajiao_count'] = user_info.get('dajiao_count', 0) + 1  # 增加打胶次数统计

                # 保存完整用户数据
                self.data_manager.update_niuniu_length(user_id, user_info)
                self.data_manager.update_group_data(str(group_id), group_data)
                # 更新上次打胶时间
                self.cool_down_time[group_][user_][last_dajiao_time] = current_time
            ...
            if change > 0:
                template = self.message_gen.get_template('dajiao', 'increase_messages')
                message = template.format(nickname=sender_nickname, change=change)
            elif change < 0:
                positive_change = -change
                template = self.message_gen.get_template('dajiao', 'decrease_messages')
                message = template.format(nickname=sender_nickname, change=positive_change)
            else:
                template = self.message_gen.get_template('dajiao', 'no_effect_messages')
                message = template.format(nickname=sender_nickname)
    ...
    async def handle_zhuli_dajiao(self, event: AstrMessageEvent, target_name: str = None):
        ...
        if group_id and user_id in group_data:
            ...
            group_ = self.cool_down_time.setdefault(group_id, {})
            user_ = group_.setdefault(user_id, {})
            last_zhuli_dajiao_time = user_.get("last_zhuli_dajiao_time", 0)
            time_diff = current_time - last_zhuli_dajiao_time

            # 十分钟内不允许打胶
            zhuli_dajiao_cd = 10
            MIN_COOLDOWN = zhuli_dajiao_cd * 60
            if time_diff < MIN_COOLDOWN:
                template = self.message_gen.get_template('zhuli_dajiao', 'cooldown_messages')
                yield event.plain_result(template.format(nickname=sender_nickname, dajiao_cd=zhuli_dajiao_cd))
                return
        ...
        user_info["length"] += change
        target_info["length"] += change_a
        user_info["assist_others"] = user_info.get('assist_others', 0) + 1
        target_info["assisted_times"] = user_info.get('assisted_times', 0) + 1
        self.cool_down_time[group_][user_]["last_zhuli_dajiao_time"] = current_time
        self.data_manager.update_niuniu_length(user_id, user_info)
        self.data_manager.update_niuniu_length(target_user_id, user_info)
        self.data_manager.update_group_data(group_id, group_data)
        yield event.plain_result(template.format(
            nickname=sender_nickname,
            ...
            target_assists=target_info.get('assist_others', 0),
        ...
        if group_id:
            ...
            if self.check_probability(double_loss_probability):
                if self.check_probability(0.7):
                    ...
                    yield event.plain_result(
                        f"{sender_nickname} 和 {target_info['nickname']}，你们俩的cow刚一碰撞，就像两颗脆弱的玻璃珠，“啪嗒”一下都折断啦！双方的cow长度都减半咯！")
                    return
                else:
            ...
            if abs(diff) <= 10:
                if self.check_probability(0.35):
                    ...
                    self.data_manager.update_group_data(group_id, group_data)
                    yield event.plain_result(
                        f"{sender_nickname} 和 {target_info['nickname']}，你们的cow长度差距不大，就像两位旗鼓相当的对手，继续加油哦！")
                    return
                ...
                if self.check_probability(0.6):
                    ...
                    yield event.plain_result(
                        f"{sender_nickname} 你的cow掏出来就把对方吓跑，但是你得意忘形，导致cow长度不变")
                    return
                    ...
                    yield event.plain_result(self.format_niuniu_message(
                        f"{sender_nickname} 你的cow气度不凡，但是你的意志不够坚定，被{av_name}诱惑，性起鹿管导致cow缩短{bonus}cm",
                        user_info["length"]))
                    return
